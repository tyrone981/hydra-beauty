...
export default async function ServiceDetailPage(props: { params: Promise<RouteParams> }) {
  await connectDB();
    ...
    <main className="container mx-auto px-4 py-10">
        ...
        <div>
          ...
          <div className="mt-4 flex flex-wrap gap-3 text-sm text-gray-600">
            <span className="rounded-full border px-3 py-1">
              {service.duration || "Durée sur demande"}
            </span>
            <span className="rounded-full border px-3 py-1">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <a
              href={`https://wa.me/237697839818?text=${encodeURIComponent(
                `Bonjour, je souhaite réserver le service: ${service.name}`
              )}`}
              target="_blank"
            ...
            <Link href="/services" className="rounded-lg border px-5 py-3">
              Retour aux services
            </Link>
          </div>
        </div>
      ...
      {related.length > 0 && (
        <section className="mt-14">
          <h2 className="mb-5 text-2xl font-semibold">Services liés</h2>
