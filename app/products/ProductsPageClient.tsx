...
export default function ProductsPageClient({ initialProducts }: ProductsPageClientProps) {
  ...
  }, [selected, query, initialProducts]);
    ...
    <div className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-24">
      ...
      >
        ...
        <div className="relative inline-block">
          <span className="absolute -top-6 left-1/2 -translate-x-1/2 font-cormorant italic text-[5rem] text-white/[0.03] leading-none pointer-events-none select-none whitespace-nowrap">
            Produits
          </span>
          <h1 className="font-cormorant italic text-5xl lg:text-6xl text-[#f0e8dd]">
            Nos <span className="text-[#b89a6e]">Produits</span>
          </h1>
        </div>
        ...
        {filtered.map((product, i) => (
          ...
          >
            <Link href={`/products/${product.slug ?? product.id}`}>
              <div className="relative h-64 overflow-hidden rounded-t-2xl bg-[#111f22]">
                {product.image ? (
                  <Image
                    src={product.image}
                    alt={product.name}
                  ...
                  <div className="w-full h-full flex items-center justify-center">
                    <span className="text-[#b89a6e]/30 text-xs tracking-widest uppercase">
                      Hydra Beauty
                    </span>
                  </div>
                )}
            ...
            <div className="p-5">
              {product.category && (
                <p className="text-[#b89a6e] text-[0.6rem] tracking-[0.2em] uppercase font-light mb-1">
                  {product.category}
                </p>
              )}
              <h3 className="font-cormorant italic text-xl text-[#f0e8dd] mb-1">
                {product.name}
              </h3>
              {product.description && (
                <p className="text-[#c8beb4] text-xs font-light leading-relaxed mb-4 line-clamp-2">
                  {product.description}
                </p>
              )}
              <div className="flex items-center justify-between">
                <span className="font-cormorant text-xl text-[#b89a6e]">
      ...
      {filtered.length === 0 && (
        <div className="text-center py-20">
          <p className="text-[#c8beb4]/50 font-light text-sm">
            {query
          ...
          {query && (
            <button
              ...
              className="mt-4 text-[#b89a6e] text-xs tracking-widest uppercase hover:underline font-light"
            >
              Réinitialiser
            </button>
          )}
      ...
      <Link
        href="/"
        className="inline-flex items-center gap-2 text-[#b89a6e]/60 text-xs tracking-[0.2em] uppercase hover:text-[#b89a6e] transition-colors font-light"
      >
